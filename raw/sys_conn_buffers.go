...
func setReceiveBuffer(c net.PacketConn) error {
	...
	if syscallConn == nil {
		return conn.SetReadBuffer(DesiredReceiveBufferSize)
	}
	...
	if size >= DesiredReceiveBufferSize {
		log.Log(logging.DebugLevel, fmt.Sprintf("Conn has receive buffer of %d kiB (wanted: at least %d kiB)", size/1024, DesiredReceiveBufferSize/1024))
		return nil
	}
	// Ignore the error. We check if we succeeded by querying the buffer size afterward.
	_ = conn.SetReadBuffer(DesiredReceiveBufferSize)
	newSize, err := inspectReadBuffer(syscallConn)
	if newSize < DesiredReceiveBufferSize {
		// Try again with RCVBUFFORCE on Linux
		_ = forceSetReceiveBuffer(syscallConn, DesiredReceiveBufferSize)
		newSize, err = inspectReadBuffer(syscallConn)
		if err != nil {
			return fmt.Errorf("failed to determine receive buffer size: %w", err)
		}
	...
	log.Log(logging.DebugLevel, fmt.Sprintf("Increased receive buffer size to %d kiB", newSize/1024))
	return nil
}
